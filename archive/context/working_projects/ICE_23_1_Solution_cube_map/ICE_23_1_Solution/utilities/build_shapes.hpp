...
//Given a vector of strings for files paths to the images for a cube map and 
// a boolean value that if true indicates the image should be flipped, creates and
// returns a cube map texture ID.
unsigned int GetCubeMap (std::vector<std::string> faces, bool flipped = false);

//Returns an NDC origin centered cube for use with cubemaps (x,y,z values only)
