...
BasicShape GetCube (VAOStruct vao) {
    ...
    new_shape.Initialize(vao,skybox_vertices,sizeof(skybox_vertices),36,GL_TRIANGLES);
    ...
    new_shape.InitializeEBO(indices,sizeof(indices),36);
...
unsigned int GetCubeMap (std::vector<std::string> faces, bool flipped){
    ...
    for (unsigned int i = 0; i < faces.size(); i++)
    {
        ...
        if (data)
        {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 
                         0, channel_type, width, height, 0, channel_type, GL_UNSIGNED_BYTE, data
            );
